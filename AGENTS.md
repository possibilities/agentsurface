...
- `main.ts` owns routing, exit semantics, and the popup-friendly failure
  hold. Routes are `launch`, `--help`, `--version`.
- `launch/model.ts` is the pure form: fields, focus, the harness → model →
  ...
  without a terminal is decided here, where tests reach it.
- `launch/app.ts` is the thin OpenTUI shell over the model plus the launch
  executor. `launch/theme.ts` holds the Signal Room tokens; `launch/overlay.ts`
  is the palette anatomy, generalized one notch for the project picker.
- `herdr.ts` speaks the herdr CLI's socket API: create workspace/worktree
  and agent start (with the pane-busy ready retry). The intent travels as a
  native token on the launch itself, so a startup dialog cannot drop it.
  JSON answers only; success on stdout, errors on stderr.
