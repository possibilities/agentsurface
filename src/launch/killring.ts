/**
 * The readline kill ring, observed rather than re-implemented: the widget
 * performs its own kills, and the shell diffs the field around each one to
 * learn what died. Consecutive kills merge into one entry — forward kills
 * append, backward kills prepend — exactly as emacs accumulates a region
 * killed in pieces. ctrl+y yanks the freshest entry; alt+y, immediately
 * after a yank, replaces it with the next-older entry around the ring.
 */
...
/** What one contiguous deletion removed: the text between the common
 * prefix and common suffix of the before and after states. */
